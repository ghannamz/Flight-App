...
public class LocalInMemoryCache implements Cache {
    private final ConcurrentHashMap<Integer, SoftReference<Object>> cache = new ConcurrentHashMap<>();
    private final DelayQueue<DelayedCacheObject> cleaningUpQueue = new DelayQueue<>();

    public LocalInMemoryCache() {
    ...
    @Override
    public void add(Integer key, Object value, long periodInMillis) {
        if (key == null) {
            return;
        }
        if (value == null) {
